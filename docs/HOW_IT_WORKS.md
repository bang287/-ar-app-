...
MindAR 會持續更新 target anchor 的位置與旋轉。為了降低手機手持時的細碎抖動，`projectMindARSession` 會把 MindAR 原始 anchor 當成追蹤來源，再建立一個平滑後的 `stableGroup` 來承載實際圖層：

- 掃到 target 時，先把 `stableGroup` 對齊原始 anchor。
- 每一幀用 `lerp` / `slerp` 平滑 position、scale、quaternion。
- target 短暫遺失時，保留最後姿態約 300ms，再暫停影片與隱藏圖層。
- 這不改 MindAR 引擎，只是在 Three.js 顯示層做穩定處理。
