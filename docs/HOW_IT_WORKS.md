...
目前 Viewer 以「先確保掃描後能顯示平台效果」為優先。掃到 Trigger Image 後，`projectMindARSession` 會建立專案圖層 mesh，並直接加入 MindAR 的 `anchor.group`：

- 掃到 target 時，載入 project layers 並加入 `anchor.group`。
- 每個 layer 保留 Editor 設定的 position、rotation、scale、Z 深度、透明度與 chroma key。
- 影片 layer 會在 target found 後播放，target lost 後暫停。
- 側面視角仍由 layer 的 Z 深度、`depthBoost` 和 `minimumDepth` 表現；目前不額外加入追蹤中介層。
...
2. 加專案版本管理，避免誤改作品。
3. 錄影加入倒數、暫停、重新錄製。
4. Editor 加吸管工具，直接從素材取色做 chroma key。
5. 圖層支援 3D model / audio / text layer。
